...
public class BaseNoise : MonoBehaviour {
    public string saveToDiskPath = "Assets/Noise";
    ...
    public virtual void Generate() {
        ...
        if (is3D) {
            viewMaterial3D.SetTexture("_MainTex", tempRenderTexture3D);
        }
        else {
            viewMaterial2D.SetTexture("_MainTex", tempRenderTexture2D);
        }
    }
    ...
    public void SaveToDisk() {
        if (is3D) {
            ...
            viewMaterial3D.SetTexture("_MainTex", AssetDatabase.LoadAssetAtPath<Texture>(path));
        }
        else {
            ...
            viewMaterial2D.SetTexture("_MainTex", AssetDatabase.LoadAssetAtPath<Texture>(path));
